...
function merged() {
    return JSON.parse(readFileSync(join(prDir, 'review.json'), 'utf8'));
}
beforeEach(() => {
...
function writeJudgment(value) {
    writeFileSync(join(prDir, 'judgment.json'), `${JSON.stringify(value, null, 2)}\n`);
}
function goodJudgment() {
    return JSON.parse(readFileSync(join(fixtures, 'judgment', 'pr-fake-1.json'), 'utf8'));
}
describe('cockpit judge-prompt', () => {
    ...
    it('does not treat a placeholder inside the diff as one of its own', () => {
        ...
        const filled = buildPrompt(document, { documentPath: 'review.json', judgmentPath: 'judgment.json' }, template);
    ...
    it('inlines the compact view at the end', () => {
        ...
        expect(prompt.indexOf('# Compact view — northwind-labs/tenant-platform#1234')).toBeGreaterThan(prompt.indexOf('## The schema your file must match'));
    });
